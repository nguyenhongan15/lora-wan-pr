"""ARCHIVED — Stage 2 LightGBM retrain + evaluation report.

Vị trí: archive/stage2-lightgbm/scripts/retrain_and_report.py
Đây là code Stage 2 LightGBM tôi tự build trước handoff cho dev ML mới
(giữ làm reference, không deploy). Path đã rebase về archive/stage2-lightgbm/.
Pyproject + src + lora_ml_predict workspace dep KHÔNG còn install ở root —
muốn chạy lại cần `uv pip install -e archive/stage2-lightgbm` (chưa test
post-archive). Xem archive/stage2-lightgbm/README.md.
...
# archive/stage2-lightgbm/scripts/retrain_and_report.py
#   parents[0] = scripts/, parents[1] = stage2-lightgbm/, parents[2] = archive/, parents[3] = repo root
_REPO_ROOT = Path(__file__).resolve().parents[3]
_ARCHIVE_DIR = Path(__file__).resolve().parents[1]
# evaluation/* không phải installed package → cần thêm dir chứa nó vào sys.path
...
if str(_ARCHIVE_DIR) not in sys.path:
    sys.path.append(str(_ARCHIVE_DIR))
...
_REPORTS_ROOT = _ARCHIVE_DIR / "evaluation" / "reports"
