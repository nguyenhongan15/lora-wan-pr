...
class GoongHttpClient:
    ...
    def search(self, query: str) -> AddressLookupResult | None:
        ...
        try:
            ...
            raise GeocodingProviderUnavailable(f"goong network: {e}") from e

        if resp.status_code == 429:
            raise GeocodingProviderUnavailable("goong rate-limited (429)")
        if resp.status_code in (401, 403):
            raise GeocodingProviderUnavailable(
                f"goong auth failed ({resp.status_code})"
            )
        if resp.status_code >= 500:
            raise GeocodingProviderUnavailable(f"goong 5xx: {resp.status_code}")
        if resp.status_code != 200:
            raise GeocodingProviderUnavailable(
                f"goong unexpected status {resp.status_code}"
            )

        try:
            data = resp.json()
        except ValueError as e:
            raise GeocodingProviderUnavailable(f"goong non-json: {e}") from e
