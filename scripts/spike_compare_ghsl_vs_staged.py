...
GHSL_TILE = REPO_ROOT / "tmp" / "ghsl" / "GHS_BUILT_H_AGBH_E2018_GLOBE_R2023A_54009_100_V1_0_R8_C29.tif"
...
def main() -> int:
    ...
    with rasterio.open(GHSL_TILE) as ghsl_ds, \
            rasterio.open(STAGED_DSM) as dsm_ds, \
            rasterio.open(DTM) as dtm_ds:
        print("== Per-cell comparison ==")
        print(f"{'cell':<18} {'lat':>10} {'lon':>10} {'GHSL_h':>8} {'StageD_excess':>14} {'diff':>7}")
        print("-" * 75)
        for name, lat, lon in test_points:
            ...
            excess = dsm_v - dtm_v if math.isfinite(dsm_v) and math.isfinite(dtm_v) else float("nan")
            diff = ghsl_h - excess if math.isfinite(ghsl_h) and math.isfinite(excess) else float("nan")
            print(f"{name:<18} {lat:>10.5f} {lon:>10.5f} "
                  f"{ghsl_h:>8.2f} {excess:>14.2f} {diff:>7.2f}")
        ...
        print(f"  GHSL building height  : mean={a.mean():.2f} m, p50={np.median(a):.2f}, "
              f"p90={np.percentile(a, 90):.2f}, p99={np.percentile(a, 99):.2f}, max={a.max():.2f}")
        print(f"  Stage D excess        : mean={b.mean():.2f} m, p50={np.median(b):.2f}, "
        ...
        print(f"  Pixels with GHSL h > 0.5m   : {nonzero_ghsl} ({100*nonzero_ghsl/len(a):.1f}%)")
        print(f"  Pixels with StageD ex > 0.5m: {nonzero_excess} ({100*nonzero_excess/len(b):.1f}%)")
